  ...
  if (tmp_str != s21_NULL) {
    switch (parameters->specifier) {
      ...
      case 'p':
        ...
        break;
      case 's':
        s21_str_to_str(input, tmp_str, parameters);
        break;
      case 'u':
...
char *flt_to_str(va_list *args, char *str, s21_FORMAT *parameters) {
  ...
  if ((isinf(arg) || isnan(arg) || arg == 0) &&
    ...
    // добавление "." при нулевой точности
    if (parameters->flag_sharp && parameters->precision == 0)
      s21_strcat(str, ".");
    // добавление суффикса нотации
    if (s21_strchr("eE", parameters->specifier) ||
...
// сдвиг строки на 1 позицию вправо
...
// обработка ширины форматной строки
...
char *s21_str_to_str(va_list *args, char *str, s21_FORMAT *cs) {
  ...
  // дополнить пробелами до заданной ширины и выровнять вывод по левому или
  // правому краю
...
// Кол-во символов до появления %n
